...
const createPokemonLookup = (pokeDex) => {
  const pokemonLookup = {};
  for (const generation in pokeDex) {
    const pokemonList = pokeDex[generation];
    for (const pokemon of pokemonList) {
      const normalizedPokemonName = pokemon.name.toLowerCase();
      const urlParts = pokemon.url.split("/").filter(Boolean);
      const pokemonNumber = urlParts[urlParts.length - 1];
      pokemonLookup[normalizedPokemonName] = pokemonNumber;
    }
  }
  return pokemonLookup;
};

// Preprocessed Pokémon lookup dictionary
const pokemonLookup = createPokemonLookup(pokeDex);

// Function to find Pokémon number by name using the preprocessed lookup dictionary
export const findPokemonNumberByName = (name) => {
  const normalizedPokemonName = name.toLowerCase();
  return pokemonLookup[normalizedPokemonName] || null;
};

// export const pokedexTemp = [
//   {
//     name: "bulbasaur",
//     url: "https://pokeapi.co/api/v2/pokemon/1/",
//   },
